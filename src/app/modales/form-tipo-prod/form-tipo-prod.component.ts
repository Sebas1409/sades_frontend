...
import { TipoProducto } from 'src/app/interfaces/tipo_producto';
...
@Component({
  selector: 'app-form-tipo-prod',
  templateUrl: './form-tipo-prod.component.html'
})
export class FormTipoProdComponent implements OnInit {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: paraModal, public dialogRef: MatDialogRef<FormTipoProdComponent>,
  ...
  async ngOnInit() {
    ...
    if (this._edicion) {
      this._titulo = "Editar Tipo de Producto";
      this.item = this.data.item;
    } else {
      this._titulo = "Registar Tipo de Producto";
    }
  }
  ...
  registrar_item() {
    ...
    this.fmysql.registrar(environment.productos.tipos_producto, datos).subscribe(res => {
  ...
  modificar_item() {
    ...
    let datos: formatoModificacion = {
      idcampo: { nombre: 'idtipoprod', valor: this.item.idtipoprod },
      campos: [
        'nombre',
        'modificacion'
      ],
    ...
    this.fmysql.modificar(environment.productos.tipos_producto, datos).subscribe(res => {
      console.log('Respues de API:', res);
      this.funciones.terminar_loader();
      if (res.ok) {
        this.cerrar_ventana(true);
