...
export class FormMarcasComponent implements OnInit {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: paraModal, public dialogRef: MatDialogRef<FormMarcasComponent>,
  ...
  registrar_item() {
    ...
    this.fmysql.registrar(environment.productos.marcas, datos).subscribe(res => {
  ...
  modificar_item() {
    ...
    this.fmysql.modificar(environment.productos.marcas, datos).subscribe(res => {
      console.log('Respues de API:', res);
      this.funciones.terminar_loader();
      if (res.ok) {
        this.cerrar_ventana(true);
